...
Several studies indicate sufficient variations in the hydraulic properties between siliceous and calcareous sand that would lead to different transport mechanisms of the sediments. Despite these observations and to our knowledge, sediment transport models for calcareous sands under wave and current actions have not yet been proposed.

In pyReef, sediment transport induced by wave and current dynamic is derived from Soulsby and Van Rijn formulation \citep{Soulsby97}. The method computes the total load transport rate resulting from the addition of bed load and suspended load for each type of loose carbonate sands initially set by the user. The model also assumes that each of sand particles are perfectly sorted. The method suits well with our approach as it can be applied in a quasi-steady form. The transport rate is defined by:
\begin{equation}
...
\noindent The threshold current velocity ($\bar{u_{cr}}$) of motion follows:
\begin{equation}
